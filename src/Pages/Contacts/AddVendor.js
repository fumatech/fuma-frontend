...
const AddVendor = () => {
  ...
  const validateEmail = async (value) => {
    ...
    try {
      ...
      if (res.status === 200) {
        ...
        setEmailError(""); // no error — email is available
      } else {
  ...
  const validateVendorId = async (value) => {
    if (!value) {
      setVendorIdError("");
      return;
    }
    ...
    try {
      const res = await fetch(
        `${process.env.REACT_APP_BASE_URL}/vendor/check-vendorid?vendorId=${value}`
      );
      if (res.status === 200) {
      ...
      console.error("Vendor ID check failed:", err);
    }
  };
