...
function EditSoSale() {
  ...
  const [totalSaleUnits, setTotalSaleUnits] = useState(0);
    ...
    const fetchPurchaseData = async () => {
      try {
        const response = await fetch(
          `${process.env.REACT_APP_BASE_URL}/sale-so-order/get/${id}`
        );
        ...
        setPurchaseReferenceNumber(purchase.orderRefernceNumber);
        setOrderedBy(purchase.orderedBy);
        setAddedBy(purchase.addedBy);
        setFranchiseName(purchase.franchise);
        ...
        // Fetch franchise (customer) details by customerId
        axios
        ...
        const selectedProducts = purchase.saleSoItem.map((item) => {
          ...
          return {
            ...
            selectedTax: matchedTaxOption || null, // ✅ This is what Select expects
          };
        });

        setSelectedProducts(selectedProducts);
        setSelectedVariations({});
        setProductsData(purchase.saleSoItem);

        // Shipping Details
        if (
          purchase.shippingSoDetails &&
          purchase.shippingSoDetails.length > 0
        ) {
          const shippingDetail = purchase.shippingSoDetails[0]; // Assuming there's only one shipping detail
  ...
  useEffect(() => {
    if (selectedOrderId) {
      axios
        .get(
          `${process.env.REACT_APP_BASE_URL}/purchaseorder/getPoDataById/${orderId}`
        )
        .then((response) => {
          ...
          const fetchProductDetails = data.orderItems.map((item) => {
            return axios
              ...
              .then((response) => {
                ...
                const updatedQuantity = item.updatedQuantity || 1;
          ...
          Promise.all(fetchProductDetails)
            .then((productsWithPrices) => {
              ...
              const totalSaleUnits = productsWithPrices.reduce(
                (total, product) => total + product.updatedQuantity,
                0
              );

              //  setTotalAmount(totalAmount);
              setTotalUnits(totalUnits);
              setTotalSaleUnits(totalSaleUnits);
            })
            .catch((error) => {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `${process.env.REACT_APP_BASE_URL}/purchaseorder/getOrderIdsByStatus/1`
      )
      .then((response) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `${process.env.REACT_APP_BASE_URL}/sale-so-order/update/${id}`,
        //
        {
          method: "PUT",
          headers: {
            ...
            <div className="container-fluid">
              <div className="row mb-2">
                <div className="col-sm-12 d-flex align-items-center flex-wrap gap-2">
                  <h1 className="all-heading mb-0 me-3">Edit So Sale</h1>
                  <span>
