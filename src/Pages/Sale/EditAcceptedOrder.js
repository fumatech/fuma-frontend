  ...
  useEffect(() => {
    ...
    if (email) {
      ...
      fetch(`${process.env.REACT_APP_BASE_URL}/user/username?email=${email}`)
        ...
        .then((data) => {
          if (data) {
            setAddedBy(data); // Set the username in state
          }
        })
  ...
  useEffect(() => {
    const fetchPurchaseData = async () => {
      try {
        ...
        setFranchiseName(purchase.franchiseName);
        ...
        const selectedProducts = purchase.franchiseOrderItems.map((item) => ({
          ...
          updatedQuantity: item.updatedQuantity ?? item.quantity, // Default shipping quantity
        ...
        const totalBeforeEdit = purchase.franchiseOrderItems.reduce(
            ...
            <div className="container-fluid">
              <div className="row mb-2">
                <div className="col-sm-6">
                  <h1 className="all-heading">Ship Accepted Order</h1>
                </div>
              </div>
            </div>
          </section>
