...
function AddSoSale() {
  ...
  useEffect(() => {
    axios
      .get(
        `${process.env.REACT_APP_BASE_URL}/sale-so-order/next-reference-number`
      )
      .then((res) => {
  ...
  useEffect(() => {
    if (selectedOrderId) {
      axios
        .get(
          `${process.env.REACT_APP_BASE_URL}/franchisepurchaseorder/getPoDataById/${selectedOrderId.value}`
        )
        .then((response) => {
          ...
          setTotalSaleUnits(data.totalShippedItems);

          // Fetch franchise (customer) details by customerId
          axios
            .get(
              `${process.env.REACT_APP_BASE_URL}/customer/${data.customerId}`
            )
            .then((res) => {
          ...
          // Handle product items (as you already do)
          const fetchProductDetails = data.franchiseOrderItems.map((item) => {
            return axios
              ...
              .catch(() => ({
                ...
                quantity: item.updatedQuantity || 1,
                discountPercent: 0,
                profitMargin: 0,
                lineTotal: 0,
              }));
          ...
          Promise.all(fetchProductDetails).then((productsWithPrices) => {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `${process.env.REACT_APP_BASE_URL}/franchisepurchaseorder/getOrderIdsByStatus/3`
      )
      .then((response) => {
        const options = response.data.map((id) => ({ value: id, label: id }));
        setOrderId(options);
        if (franchisePurchaseOrderId) {
          const foundOrder = options.find(
            (option) => option.label === franchisePurchaseOrderId
          );
          if (foundOrder) {
            setSelectedOrderId(foundOrder);
          }
        }
      })

      .catch((error) => console.error("Error fetching order IDs:", error));
  }, [franchisePurchaseOrderId]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `${process.env.REACT_APP_BASE_URL}/sale-so-order/save`,
        {
          method: "POST",
          headers: {
